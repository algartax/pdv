...
async function saveAgendamento(appointmentData) {
    try {
        ...
        const agendamentoData = {
            ...
            profissional: appointmentData.professionalName || null,  // AQUI - COLUNA PROFISSIONAL RECEBE O NOME SELECIONADO
            group_id: appointmentData.group_id || null  // NOVO CAMPO PARA AGRUPAMENTO
        };
...
async function loadAgendamentosFromDB(startDate, endDate) {
    try {
        ...
        return data.map(agendamento => ({
            ...
            group_id: agendamento.group_id || null  // INCLUIR GROUP_ID
        }));
...
async function updateAgendamento(id, appointmentData) {
    try {
        ...
        const updateData = {
            ...
            status: appointmentData.status || 'agendado',
            observacoes: appointmentData.notes || null,
            profissional: appointmentData.professionalName || null,  // AQUI TAMBÉM - ATUALIZA A COLUNA PROFISSIONAL
            group_id: appointmentData.group_id || null  // INCLUIR GROUP_ID NA ATUALIZAÇÃO
        };
...
/**
 * Verificar conflito de horário - USANDO A COLUNA PROFISSIONAL
 */
async function verificarConflitoHorario(professionalName, data, horaInicio, excludeId = null) {
    try {
        ...
        let query = supabaseClient
            ...
            .eq('id_empresa', window.currentCompanyId)
            .eq('profissional', professionalName)  // AQUI - BUSCA PELA COLUNA PROFISSIONAL
        ...
        const novoHorarioMinutos = timeToMinutes(horaInicio.substring(0, 5));
        ...
        return agendamentos.some(agendamento => {
            ...
            return novoHorarioMinutos >= inicioExistente && novoHorarioMinutos < fimExistente;
        });
...
console.log('Sistema de integração com Supabase carregado - Nome do profissional vai para coluna PROFISSIONAL');