// agenda-servicos-agend.js - Formulários e Agendamento
// ====================================================
...
Object.assign(AgendaModals.prototype, {
    ...
    validateDateInput() {
        ...
        if (date) {
            ...
            if (selectedDate < today) {
                this.showFieldError(this.elements.dateInput, 'Data não pode ser no passado');
                return false;
            }
        }
    ...
    validateUserPermissions(formData) {
        ...
        if (window.currentUser && window.currentUser.funcao === 'funcionario') {
            // Funcionário só pode agendar para si mesmo
            if (formData.professional !== window.currentUser.auth_user_id) {
        ...
        try {
            ...
            for (const selectedTime of formData.times) {
                const conflict = existingAppointments.find(apt => {
                    ...
                    return this.hasTimeOverlap(selectedTime, apt.time);
                });
                
                if (conflict) {
                    const professionalName = formData.professionalName || 'o profissional selecionado';
                    this.showError(`Conflito de horário: ${professionalName} já possui um agendamento às ${selectedTime} com ${conflict.client}.`);
                    return false;
                }
            }
    ...
    async getExistingAppointments(professionalId, date, excludeId = null) {
        try {
            const client = supabase.createClient(SUPABASE_URL, SUPABASE_ANON_KEY);
            
            let query = client
                .from('agendamentos')
                .select('id, profissional, data, horario, cliente')
                .eq('id_empresa', window.currentCompanyId)
                .eq('profissional', professionalId)
                .eq('data', date)
    ...
    hasTimeOverlap(time1, time2) {
        ...
        // Considerar sobreposição se a diferença for menor que o intervalo de slot
        const slotInterval = this.agenda?.settings?.slotInterval || 30;
        return Math.abs(minutes1 - minutes2) < slotInterval;
    },
...
// Gerenciador de serviços será carregado do arquivo agenda-servicos-manager.js